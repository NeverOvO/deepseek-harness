...
| 发布集边界 | `packages/*/*` 与 `vendor/*` 的全部成员，加上 `apps/cli` 和 `apps/web`；精确的 Private Native Allowlist 排除 `apps/desktop` |
