...
The v0.1 carrier runs the built `@deepseek-ai/dsh` CLI as a child process. In a source checkout it prefers the package-manager Node inherited through `npm_node_execpath`, so native DSH dependencies keep the ABI they were installed for. A packaged application has no package-manager Node and therefore reuses Electron's executable with `ELECTRON_RUN_AS_NODE=1`; Forge rebuilds packaged native dependencies for that Electron ABI. `DSH_DESKTOP_NODE` remains an explicit launcher override for diagnostics.

The child boots the Web profile with `--host 127.0.0.1 --port 0`. The desktop shell displays a local Yanami startup surface immediately, treats the existing `dsh web: http://127.0.0.1:<port>` line as the readiness boundary, and then replaces the startup surface with that runtime URL in one `BrowserWindow`. Application shutdown sends SIGTERM to the runtime and escalates to SIGKILL only after a bounded teardown timeout.
