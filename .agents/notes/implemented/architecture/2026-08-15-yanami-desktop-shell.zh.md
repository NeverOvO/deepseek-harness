...
v0.1 的承载方式是：使用 Electron 自身可执行文件并设置 `ELECTRON_RUN_AS_NODE=1`，把构建后的 `@deepseek-ai/dsh` CLI 作为子进程启动。子进程以 `--host 127.0.0.1 --port 0` 启动 Web Profile。桌面壳把现有的 `dsh web: http://127.0.0.1:<port>` 输出作为 Ready 边界，随后由单一 `BrowserWindow` 加载该地址。应用退出时先向 Runtime 发送 SIGTERM，只在限定时间内无法正常退出时升级为 SIGKILL。

Renderer 保持低权限：关闭 Node Integration，开启 Context Isolation 与 Sandbox，禁止导航到当前私有 DSH Runtime Origin 之外的地址；Preload 当前只暴露一个只读的桌面身份对象。普通 HTTP(S) 链接交给系统默认浏览器打开，而不是新建 Electron Window。
...
第一版内部仍然通过私有 Loopback HTTP 承载 Web Profile。因为 Server 只绑定本机回环地址，并由 Electron 完整管理生命周期，所以 v0.1 可以接受这一点，但 Native IPC Carrier 仍然是后续优化方向。当前分发还没有达到生产发布状态：品牌应用图标、Apple Code Signing/Notarization、Windows Maker/Signing、自动更新和 Packaged Build 验证仍属于后续工作。
