...
`src/styles/` holds six sheets imported by the web shell's `base.css`: `base.css`, `design-platform.css`, `yanami-workbench.css`, `scrollbar.css`, `gradient-shadow-text.css`, and `shiki.css`. `yanami-workbench.css` is the deployment-level Yanami Workbench visual layer: it overrides semantic aliases after the base design palette and owns the additional `--dsw-alias-yanami-*` tokens consumed by shell/layout packages. It supplies matching light and dark variants rather than replacing the existing Appearance preference. `scrollbar.css` is the sole consumer of the `--dsw-alias-scrollbar-*` tokens and must follow the palette sheets that declare or override them.
...
- **Yanami Workbench is currently a deployment visual layer** — light/dark/system still select palette semantics, but there is no separate switch that removes the Yanami overrides. If multiple branded workbenches must coexist, move this layer behind a registered theme/profile.
...
- **The token sheets are the sole color authority** — feature packages consume semantic aliases and do not copy palette literals. Design-owner-approved additions enter the owning theme sheet first, then feature packages consume the new alias.
